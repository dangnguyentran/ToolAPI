...
def main():
    ...
    parser.add_argument('--auto-upgrade-pet', dest='auto_upgrade_pet', default='n',
                        choices=['y', 'n'], help='Auto Upgrade All Pet? (default n) (y/n)')
    parser.add_argument('--max-level-pet', dest='max_level_pet', type=int, default=10,
                        help='Masukkan max level upgrade (default 10)')
    parser.add_argument('--auto-daily-combo', dest='auto_daily_combo', default='n',
                        choices=['y', 'n'], help='Auto Daily Combo? (default n) (y/n)')
    parser.add_argument('--user-input-order', dest='user_input_order', type=str,
                        help='Masukkan urutan Daily Combo (pisahkan dengan koma, misal: 1,4,3,2)')
    
    args = parser.parse_args()
    
    auto_upgrade_pet = args.auto_upgrade_pet
    max_level_pet = args.max_level_pet
    auto_daily_combo = args.auto_daily_combo
    user_input_order = [int(x.strip()) for x in args.user_input_order.split(',')] if args.user_input_order else None
    ...
    while True:
        ...
        try:
            ...
            try:
                with open('querypixeltap.txt', 'r') as file:
