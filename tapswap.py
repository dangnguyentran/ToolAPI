...
with open('init_datatapswap.txt', 'r') as file:
...
parser = argparse.ArgumentParser(description='Chương trình tự động tùy chọn')

parser.add_argument('--use_booster', choices=['Y', 'N'], default='N', help="Sử dụng booster tự động? (Y/N)")
parser.add_argument('--use_upgrade', choices=['Y', 'N'], default='N', help="Nâng cấp tự động? (Y/N)")
parser.add_argument('--use_kyc', choices=['Y', 'N'], default='N', help="Auto KYC Task Binance? (Y/N)")
parser.add_argument('--auto_clear', choices=['Y', 'N'], default='N', help="Auto clear task? (Y/N)")
parser.add_argument('--auto_claim_league', choices=['Y', 'N'], default='N', help="Tự động nhận phần thưởng League? (Y/N)")

args = parser.parse_args()

use_booster = args.use_booster
use_upgrade = args.use_upgrade
use_kyc = args.use_kyc
auto_clear = args.auto_clear
auto_claim_league = args.auto_claim_league

print(f"Sử dụng booster tự động? (Y/N): {use_booster}")
print(f"Nâng cấp tự động? (Y/N): {use_upgrade}")
print(f"Auto KYC Task Binance? (Y/N): {use_kyc}")
print(f"Auto KYC Task Binance? (Y/N): {auto_clear}")
print(f"Tự động nhận phần thưởng League? (Y/N): {auto_claim_league}")
...
def finish_mission_item(access_token, mission_id, item_index, user_input=None):
    url = "https://api.tapswap.ai/api/missions/finish_mission_item"
    headers = {
            "Authorization": f"Bearer {access_token}",
            "Accept": "*/*",
            "Accept-Language": "en-US,en;q=0.9",
...
def finish_mission(access_token, mission_id):
    url = "https://api.tapswap.ai/api/missions/finish_mission"
    headers = {
            "Authorization": f"Bearer {access_token}",
            "Accept": "*/*",
            "Accept-Language": "en-US,en;q=0.9",
